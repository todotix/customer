...
class GeneralNodeTest extends TestCase {
    ...
    public function testNodesItem() {
        ...
        foreach(\Todotix\Master\App\Node::whereNotIn('type', ['subchild', 'field'])->with('fields','fields.field_options')->get() as $node){
    ...
    public function testNodesDeleteItems() {
        $user = \App\User::find(1);
        foreach(\Todotix\Master\App\Node::whereNotIn('type', ['subchild', 'field'])->orderBy('id', 'DESC')->with('fields','fields.field_options')->get() as $node){
            $model = \FuncNode::node_check_model($node);
            if($node->name!='field'&&$node->name!='node'&&$last = $model->orderBy('id', 'DESC')->first()){
